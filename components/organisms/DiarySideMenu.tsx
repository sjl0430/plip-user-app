...
export function DiarySideMenu({ open, onClose }: DiarySideMenuProps) {
  ...
  return (
    <AnimatedSideSheet open={open} onClose={onClose} aria-label="다이어리 메뉴" className="plip-diary-sheet">
      <div className="plip-diary-sheet__top">
        <p className="plip-diary-sheet__title">Menu</p>
        <button type="button" className="plip-diary-sheet__close" aria-label="닫기" onClick={onClose}>
          ✕
        </button>
      </div>
          ...
          {cells.map((cell, index) =>
            cell.date && !cell.outside ? (
              <TextLink
                ...
                className={`plip-diary-calendar__day ${cell.day === 11 ? "is-selected" : ""}`}
                onClick={onClose}
              >
                {cell.day}
              </TextLink>
            ) : (
              <span key={`${cell.day}-${index}`} className="plip-diary-calendar__day is-outside">
                {cell.day}
              </span>
            ),
          )}
