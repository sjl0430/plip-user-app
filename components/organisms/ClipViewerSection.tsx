...
export function ClipViewerSection({ clipId, mode = "view" }: ClipViewerSectionProps) {
  ...
  return (
    <section className="relative flex min-h-full flex-1 flex-col bg-black" aria-label="그룹영상 뷰어">
      {clip.coverSrc ? (
        <Image src={clip.coverSrc} alt="" fill className="object-cover" sizes="430px" priority />
      ) : (
        <div className="absolute inset-0" style={{ background: clip.gradient }} aria-hidden />
      )}
      ...
      <header className="relative z-10 flex items-start gap-3 px-4 py-3">
        ...
        <div className="min-w-0 flex-1 text-white">
          ...
          <p className="mt-1 text-[12px] text-white/75">8월 14일 · #기상 #러닝</p>
        </div>
      ...
      <div className="relative z-10 mt-auto flex flex-col gap-1 px-6 pb-10 text-white">
        <p className="m-0 text-[18px] font-semibold">새벽 러닝 완료</p>
        <p className="m-0 text-[13px] text-white/80">{clip.authorName} · 07:12 업로드</p>
      </div>
