...
export function ForgotPasswordForm() {
  ...
  if (sent) {
    return (
      <div className="flex w-full flex-col gap-4">
        <h2 className="m-0 text-[28px] font-bold leading-[34px] text-[var(--dl-color-text-primary)]">재설정 메일을 보냈어요</h2>
        ...
        <button type="button" className="!text-[var(--dl-color-text-brand)] text-sm font-medium leading-5 !no-underline hover:!underline text-center text-[13px]" onClick={() => setSent(false)}>
          메일을 받지 못했나요?  다시 보내기
        </button>
      </div>
    );
  ...
  return (
    ...
    >
      <h2 className="m-0 text-[28px] font-bold leading-[34px] text-[var(--dl-color-text-primary)]">비밀번호를 잊으셨나요?</h2>
      <p className="m-0 text-sm font-normal leading-5 text-[var(--dl-color-text-secondary)]">가입한 이메일로 재설정 링크를 보내드릴게요.</p>
      <AuthField
        id="reset-email"
        name="email"
        type="email"
      ...
      <p className="text-center text-[13px] text-[var(--dl-color-text-brand)]">
        이메일이 기억났나요?{" "}
        <TextLink href={ROUTES.login} className="!text-[var(--dl-color-text-brand)] text-sm font-medium leading-5 !no-underline hover:!underline text-[13px]">
          로그인
        </TextLink>
      </p>
    </form>
